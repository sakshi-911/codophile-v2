...
export default function ClientEditor({
  ...
  const [logs, setLogs] = useState<
    { type: "log" | "error" | "warn"; message: string }[]
  >([]);
...
${component}`;
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data?.type === "console-message") {
        setLogs((prev) => [
          ...prev,
          { type: event.data.level, message: event.data.args.join(" ") },
        ]);
  ...
  useEffect(() => {
    ...
    setLogs((prev) => [
      ...prev,
      { type: "log", message: "--- Reloading Preview ---" },
    ]);
    ...
    return (
      <div className="min-h-screen bg-[#030014] text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/effects" className="text-pink-400 hover:underline">
            Back to Effects
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="h-dvh bg-[#030014] text-white font-sans flex flex-col overflow-hidden">
      ...
      <div className="h-6 bg-[#020202] border-b border-white/5 flex items-center justify-center shrink-0 z-50">
        <span className="text-[10px] font-medium text-white/20 tracking-[0.2em] uppercase select-none">
          Codophile Editor
        </span>
      </div>
      ...
      >
        {/* LEFT SIDE: PREVIEW */}
        <div
          className={`flex flex-col bg-[#050505] border-r border-white/5 relative ${
            layout === "editor" ? "hidden" : "flex-initial"
          }`}
          style={
            layout === "split"
              ? { flexBasis: `${splitPosition}%`, flexGrow: 0, flexShrink: 0 }
              : { flexGrow: 1 }
          }
        >
          {/* Toolbar */}
                ...
                <div className="flex-1 overflow-y-auto p-2 font-mono text-xs custom-scrollbar space-y-1">
                  {logs.length === 0 ? (
                    ...
                    logs.map((log, idx) => (
                      <div
                        key={idx}
                        className={`px-2 py-0.5 border-b border-white/5 ${
                          log.type === "error"
        ...
        <div
          className={`flex flex-col bg-[#0f0f0f] ${
            layout === "preview" ? "hidden" : "flex-initial"
          }`}
          style={{ flexGrow: 1 }}
        >
          {/* Editor Tabs */}
          <div className="h-14 border-b border-white/5 flex items-center justify-between px-2 bg-[#0a0a0a] shrink-0">
            ...
            <div className="flex items-center gap-2 pr-2">
              ...
              <button
                ...
                title={
                  layout === "editor" ? "Exit Fullscreen" : "Fullscreen Editor"
                }
              >
                {layout === "editor" ? (
...
function TabButton({ active, onClick, icon: Icon, label, color }: any) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 px-6 h-full border-b-2 transition-colors ${
        active
